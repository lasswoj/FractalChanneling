...
\subsection{White flows - Tabula rasa - blank slate}\label{philo:white:flow}
White flows are depiction of the newcommer onboarding process. 
\subsection{Gray flows (bare metal)}\label{philo:gray:flow}
Delivery
\subsection{Yellow flows (processing)}\label{philo:Yellow:flow}
Testing -> Kaue
\subsection{Green flows (alive)}\label{philo:green:flow}
...
\end{itemize}
\subsection{Red flows (danger)}\label{philo:red:flow}
...
\end{itemize}
\subsection{Brown flows (incident handling)}\label{philo:brown:flow}\
